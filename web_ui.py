...
MAIL_TOKENS_FILE = RESULTS_DIR / "mail_tokens.json"
try:
    _mail_tokens = json.loads(MAIL_TOKENS_FILE.read_text(encoding="utf-8"))
    if not isinstance(_mail_tokens, dict):
        _mail_tokens = {}
...
def _save_mail_tokens():
    temp_file = MAIL_TOKENS_FILE.with_suffix(".json.tmp")
    temp_file.write_text(
        json.dumps(_mail_tokens, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    temp_file.replace(MAIL_TOKENS_FILE)


def _get_mail_token(account_id, email_addr):
    email_addr = str(email_addr or "").strip().lower()
    if not account_id or "@" not in email_addr:
        return ""
    with _token_lock:
        for token, target in _mail_tokens.items():
            if not isinstance(target, dict):
                continue
            if (
                target.get("account_id") == account_id
                and target.get("email") == email_addr
            ):
                return token
        token = secrets.token_urlsafe(24)
        _mail_tokens[token] = {"account_id": account_id, "email": email_addr}
        _save_mail_tokens()
        return token


def _resolve_mail_token(token):
    with _token_lock:
        target = _mail_tokens.get(str(token or "").strip())
...
def api_accounts():
    ...
    for a in accounts:
        ...
        mail_email = str(a.get("icloud_email") or "").strip()
        if mail_email:
            ac["mail_token"] = _get_mail_token(a["id"], mail_email)
        safe.append(ac)
...
def api_compat_create():
    ...
    return jsonify({"success": True, "data": {
        "email": result["email"],
        "token": _get_mail_token(acc_id, result["email"]),
        "label": label,
...
def api_verification_code():
    ...
    target = _resolve_mail_token(token) if token else None
    if token and not target:
        return jsonify({"success": False, "message": "Token 不存在"}), 404

    email_addr = (
        target["email"] if target
        else request.args.get("email", "").strip().lower()
    )
    account_id = (
        target["account_id"] if target
        else request.args.get("account_id", "").strip()
    )
    ...
    if "@" not in email_addr:
        return jsonify({
            "success": False,
            "message": "请提供 token 或有效的 email 参数",
        }), 400
    ...
    message = "Token 对应邮箱中未找到 6 位验证码" if token else (
        f"未找到 {email_addr} 的 6 位验证码"
    )
...
def api_create_for_account(acc_id):
    ...
    try:
        ...
        tokens = {email: _get_mail_token(acc_id, email) for email in created}
        return jsonify({"ok":len(created)>0,"emails":created,"tokens":tokens,"created":len(created),"errors":len(errors),"error":errors[0] if errors else None})
    except Exception as e:
        _update_state(creating=False)
...
def api_create_batch():
    ...
    try:
        ...
        return jsonify({"ok":True,"total_created":total_created,"total_errors":total_errors,"results":{acc_id:[{"email":r.get("email"),"token":_get_mail_token(acc_id,r.get("email")) if r.get("ok") else "","ok":r.get("ok"),"error":r.get("error")} for r in results] for acc_id,results in all_results.items()}})
    except Exception as e:
        _update_state(creating=False)
        return jsonify({"ok":False,"error":str(e)})
...
def api_aliases():
    try:
        ...
        for alias in aliases:
            alias["token"] = _get_mail_token(
                alias.get("account_id"), alias.get("email")
            )
    ...
    if f.exists():
        ...
        for line in lines:
            ...
            if line and "@" in line:
                ...
                emails.append({"email":parts[0],"account_id":account_id,"token":_get_mail_token(account_id,parts[0]),"created_at":""})
    emails.reverse()
