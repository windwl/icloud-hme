...
def _emit_log(level, msg): _logs.append({"time":_now().strftime("%H:%M:%S"),"level":level,"msg":msg})
...
def api_update_account_cookies(acc_id):
    ...
    try:
        if data.get("source") == "chrome":
            cookies = extract_chrome_cookies()
            if not cookies:
                raise RuntimeError("Chrome 中未找到 iCloud Cookie，请先登录 iCloud")
            cookie_input = json.dumps(cookies, ensure_ascii=False)
